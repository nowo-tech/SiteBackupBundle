...
final class MigrationsStep extends AbstractSetupStep
{
    ...
    {
        try {
            $output = $this->runner->runOrFail(['doctrine:migrations:migrate', '--allow-no-migration']);

            return SetupStepResult::ok('Migrations applied.', [trim($output)]);
        } catch (Throwable $e) {
